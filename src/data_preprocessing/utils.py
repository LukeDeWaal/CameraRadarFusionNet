...
def _radar_transformation(_radar_data, height=None):
    """
...
def map_pointcloud_to_image(nusc, radar_points, pointsensor_token, camera_token, target_resolution=(None, None)):
    ...
    if radar_points.shape[0] == 21:
        pc = ProcRadarPointCloud(radar_points)
    elif radar_points.shape[0] == 3:
        pc = ProcEndRadarPointCloud(radar_points)
...
def imageplus_creation(nusc, image_data, radar_data, pointsensor_token, camera_token, height=(0, 3), \
    ...
    image_plus = radar2cam(cur_img, radar_points, radar_xyz_endpoint, clear_radar=int(clear_radar))
